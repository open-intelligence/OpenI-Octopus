...
/**
   *
   * @param {JSON} container the container json data
   * @param {JSON} env the env that the container will use
   * @api private
   */
  function assign_env_to_json_container(container, env) {
    const env_list = Object.keys(env);
    for (let i = 0; i < env_list.length; i++) {
      const env_name = env_list[i];
      if (undefined === env[env_name] || env[env_name] === null) {
        continue;
      }
...
function assign_securityCapabilities_to_json_container(container, securityCapabilities){

  if (!Array.isArray(securityCapabilities)) {
    return;
  }
  ...
  for (let i = 0; i < securityCapabilities.length; i++) {
    container.securityContext.capabilities.add.push(securityCapabilities[i]);
  }
...
exports.assign_env_to_json_container = assign_env_to_json_container

exports.resource_multiply = resource_multiply;

exports.resource_add = resource_add;

exports.assign_securityCapabilities_to_json_container = assign_securityCapabilities_to_json_container;
