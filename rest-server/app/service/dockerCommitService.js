...
class ImageCommitService extends Service {
  ...
  constructor(...args) {
    ...
    async commitImage(username,ip,containerId,imageDescription){
      const imageFactoryApi = this.imageFactoryApi;

      let imageName = this.config.docker.registry+"/user-images/"+ username;

      let url = imageFactoryApi.imageCommitPath();
  ...
  async startCheckCommitResultTimer(serviceInstance,username,containerId,imageName,imageDescription){
    ...
    let intervalTimer = setInterval(function (serviceInstance,username,containerId,imageName,imageDescription) {
