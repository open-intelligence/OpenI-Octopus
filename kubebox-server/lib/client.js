...
class Client {
  ...
  get_pods(namespace) {
    return merge({
      path   : `/api/v1/namespaces/${namespace}/pods?${this.store?this.store.getPodLabelQuery():''}`,
      method : 'GET'
    }, this.master_api);
  }
  ...
  watch_pods(namespace, resourceVersion) {
    return merge({
      path    : `/api/v1/namespaces/${namespace}/pods?watch=true&resourceVersion=${resourceVersion}&${this.store?this.store.getPodLabelQuery():''}`,
      method  : 'GET',
      headers : {
