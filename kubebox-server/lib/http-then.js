...
const cLogger = logger.child({widget_type: 'http-client'});
...
function getBody(options, cancellable = false) {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    const request = client.get(options, response => {
      ...
      cLogger.info({request,response},'time expended: %d ms',endTime - startTime);
      ...
      console.log('request:response:error:',JSON.stringify(error))
      finished = true;
